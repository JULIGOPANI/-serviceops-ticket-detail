...
- **Still to do (pass 2):** merge / split cells (`colspan`/`rowspan` are already in the model and every operation is written span-aware), sort by column, paste a range from Excel or Sheets, and full keyboard-only operation.
